...
def get_logger(log_file):
    logger = logging.getLogger(log_file) # 获取logger实例，如果参数为空则返回root logger
    logger.setLevel(logging.DEBUG)       # 指定logger的最低输出级别，默认的是WARNING
    
    # Handler用于向不同的输出端打包，有如下级别: DEBUG，INFO，WARNING，ERROR，CRITICAL
    # logging模块只会输出指定level以上的log
    fh = logging.FileHandler(log_file)   #FileHandler：发送错误信息到磁盘文件
    fh.setLevel(logging.DEBUG)
    ch = logging.StreamHandler()         #StreamHandler：发送错误信息到屏幕
