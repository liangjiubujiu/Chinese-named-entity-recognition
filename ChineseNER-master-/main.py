...
flags.DEFINE_boolean("train",       False,      "Weither train the model")
# configurations for the model
flags.DEFINE_integer("seg_dim",     20,         "Embedding size for segmentation, 0 if not used")
flags.DEFINE_integer("char_dim",    100,        "Embedding size for characters")
flags.DEFINE_integer("lstm_dim",    100,        "Num of hidden units in LSTM")
flags.DEFINE_string("tag_schema",   "iobes",    "tagging schema BIOSE or BIO")

# configurations for training
flags.DEFINE_float("clip",          5,          "Gradient clip")   
                    # clip_gradient 的引入是为了处理gradient explosion的问题, 这个参数多用于LSTM中
flags.DEFINE_float("dropout",       0.5,        "Dropout rate")
flags.DEFINE_float("batch_size",    20,         "batch size")
flags.DEFINE_float("lr",            0.001,      "Initial learning rate")
flags.DEFINE_string("optimizer",    "adam",     "Optimizer for training")
flags.DEFINE_boolean("pre_emb",     True,       "Weither use pre-trained embedding")
flags.DEFINE_boolean("zeros",       False,      "Weither replace digits with zero")
flags.DEFINE_boolean("lower",       True,       "Weither lower case")
...
# os.path.join()函数 返回值：将多个路径组合后返回 (注：第一个绝对路径之前的参数将被忽略)
...
def train():
    ...
    train_manager = BatchManager(train_data, FLAGS.batch_size)  # 默认的batch_size为20
    ...
    # limit GPU memory
    tf_config = tf.ConfigProto()  # tf.ConfigProto一般用在创建session的时候。用来对session进行参数配置
